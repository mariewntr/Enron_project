...
violin_worker %>% distinct(status_sender)

#comparison group
group_comparison <- list(
  c("Employee", "Vice President"),
  c("Employee", "Managing Director"),
  c("Employee", "President"),
  c("Employee", "Manager"),
  c("Employee", "Trader"),
  c("Employee", "Director"),
  c("Employee", "CEO"),
  c("Employee", "In House Lawyer")
)
...
ggplot(violin_worker, aes(as.factor(status_sender), email_count, fill = as.factor(status_sender))) +
  ...
  geom_boxplot(width = 0.1, outlier.shape = NA, color = "white")+
  ylim(c(0,20000))+
  stat_compare_means(comparisons = group_comparison, method = "t.test", label.y =  c(5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000),
                     size = 4, angle = 45)+
  scale_y_break(c(1000, 4000), scales = 0.3)+
  labs(title = "Comparison of the email count between 
       Jeff Dasovitch and the Enron's worker",
       x = "Source",
       y = "Email count per day") +
...
*Do the same to compared the different statuts in term of number email sending -> find a way to better arrange the stat display on the plot*
