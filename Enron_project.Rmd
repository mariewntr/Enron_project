...
To explore the number different type of employee we have in the company, we used the employeelist2 data frame. 
...
head(df_message_status[df_message_status$sender == "jeff.dasovich@enron.com", "status_sender"], 
     n=1)
```
*DO A BETTER PLOT !! one where we can display on the plot the mean*
```{r}
#statistics on the jeff dasovich email send
df_message_status %>% filter(sender == "jeff.dasovich@enron.com") %>% group_by(date) %>% count() %>% ungroup() %>% ggplot()+
  geom_boxplot(aes(y=n), fill = "darkgreen")
...
=> make an analysis per month the number of email send/receive by employee, one per month over the 3 years, try to find a way to do a dynamic plot with all in one for the shiny apps. 
