...
The above plot show us that, the employee are those who send the higher number of email in the company. The anova test show us the difference between the group is significant.

Table with the descritive statistique for each group
```{r}
...
```
In fact in average, it is the worker with an employee status who receive the higher number of email per day but the standard deviation is important suggesting the average per employee is spread. Maybe, Jeff Dasovitch is one of the most active in that group but by comparing him alone to the entire group the difference isn't show. 

When we compared to the entire worker, Jeff Dasovitch is the worker who receive in average the higher number of email in the company. 
...
In first for the year 1999.
```{r}
s1<-df_message_status %>% filter(!is.na(status_sender) & (year == 1999)) %>%
  group_by(month, status_sender)%>%
  count() %>% ggplot(aes(month, n, fill = month))+
  geom_bar(stat = "identity") +
  facet_grid(~status_sender)+
  labs(title = "Email send per month in 1999 in function of the Enron's worker status",
       x = "Month",
       y = "Email count per month")+
  scale_fill_brewer(palette = "Set3")+
  theme(legend.position = "none")
...
For the year 2000.
```{r}
s2<-df_message_status %>% filter(!is.na(status_sender) & (year == 2000)) %>%
  group_by(month, status_sender)%>%
  count() %>% ggplot(aes(month, n, fill = month))+
  geom_bar(stat = "identity") +
  facet_grid(~status_sender)+
  labs(title = "Email send per month in 2000 in function of the Enron's worker status",
       x = "Month",
       y = "Email count per month")+
  scale_fill_brewer(palette = "Set3")+
  theme(legend.position = "none")
...
For the year 2001.
```{r}
s3<-df_message_status %>% filter(!is.na(status_sender) & (year == 2001)) %>%
  group_by(month, status_sender)%>%
  count() %>% ggplot(aes(month, n, fill = month))+
  geom_bar(stat = "identity") +
  facet_grid(~status_sender)+
  labs(title = "Email send per month in 2001 in function of the Enron's worker status",
       x = "Month",
       y = "Email count per month")+
  scale_fill_brewer(palette = "Set3")+
  theme(legend.position = "none")
...
For the year 2002.
```{r}
s4<-df_message_status %>% filter(!is.na(status_sender) & (year == 2002)) %>%
  group_by(month, status_sender)%>%
  count() %>% ggplot(aes(month, n, fill = month))+
  geom_bar(stat = "identity") +
  facet_grid(~status_sender)+
  labs(title = "Email send per month in 2002 in function of the Enron's worker status",
       x = "Month",
       y = "Email count per month")+
  scale_fill_brewer(palette = "Set3")+
  theme(legend.position = "none")
...
Now we look for the email receive.
In first for the year 1999.
```{r}
r1 <- df_message_status %>% filter(!is.na(status_recipient) & (year == 1999)) %>%
  group_by(month, status_recipient)%>%
  count() %>% ggplot(aes(month, n, fill = month))+
  geom_bar(stat = "identity") +
  facet_grid(~status_recipient)+
  labs(title = "Email received per month in 1999 in function of the Enron's worker status",
       x = "Month",
       y = "Email count per month")+
  scale_fill_brewer(palette = "Set3")+
  theme(legend.position = "none")
...
For the year 2000.
```{r}
r2 <- df_message_status %>% filter(!is.na(status_recipient) & (year == 2000)) %>%
  group_by(month, status_recipient)%>%
  count() %>% ggplot(aes(month, n, fill = month))+
  geom_bar(stat = "identity") +
  facet_grid(~status_recipient)+
  labs(title = "Email received per month in 2000 in function of the Enron's worker status",
       x = "Month",
       y = "Email count per month")+
  scale_fill_brewer(palette = "Set3")+
  theme(legend.position = "none")
...
For the year 2001.
```{r}
r3 <- df_message_status %>% filter(!is.na(status_recipient) & (year == 2001)) %>%
  group_by(month, status_recipient)%>%
  count() %>% ggplot(aes(month, n, fill = month))+
  geom_bar(stat = "identity") +
  facet_grid(~status_recipient)+
  labs(title = "Email received per month in 2001 in function of the Enron's worker status",
       x = "Month",
       y = "Email count per month")+
  scale_fill_brewer(palette = "Set3")+
  theme(legend.position = "none")
...
For the year 2002.
```{r}
r4 <- df_message_status %>% filter(!is.na(status_recipient) & (year == 2002)) %>%
  group_by(month, status_recipient)%>%
  count() %>% ggplot(aes(month, n, fill = month))+
  geom_bar(stat = "identity") +
  facet_grid(~status_recipient)+
  labs(title = "Email received per month in 2002 in function of the Enron's worker status",
       x = "Month",
       y = "Email count per month")+
  scale_fill_brewer(palette = "Set3")+
  theme(legend.position = "none")
...
rm(s1, s2, s3, s4, r1, r2, r3, r4, p3, p4, jeff_stat, recipient_stat, statuts_stat, violin_plot, violin_plot1, violin_plot2, violin_worker)
```
