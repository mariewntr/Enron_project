...
```{r}
df_message_status %>% group_by(year,month, sender, subject)%>%
  count() %>%
...
For that, we start by counting the number of eamil send per each worker and return the 10 persons who send the highest number.
```{r}
...
statuts_stat <- df_message_status %>% filter(status_sender == "Employee") %>% 
...
**- Compared to other worker with an employee it significantly send more email but it received less.**
...
email_subject %>% select(year_month, starts_with("sum_topic_")) %>%
...
```{r fig.width=12}
#display the different topic trend in the email subject over the study's period
email_subject %>% select(year_month, starts_with("sum_email_")) %>%
  #change the orientation of the data set
  pivot_longer(
  cols = 2:8,
  names_to = "email",
...
```{r}
status_email_subject <- df_message_status %>% distinct(date, sender, status_sender, subject, reference) %>%
  mutate(#count the number of email which contain at least one word in the list of each topic
    topic_meeting = if_else(str_detect(subject, topic_meeting), 1, 0),
    topic_business_process = if_else(str_detect(subject, topic_business_process), 1, 0),
    topic_core_business = if_else(str_detect(subject, topic_core_business), 1, 0),
  ...
  p <- status_email_subject %>% filter(status_sender == status) %>%
       ...
       facet_grid(~year)+
         labs(fill = "Email content key words",
           title = paste("Email send about Enron's event and function by", status),
...
- in general the email send by each status are about meeting or business process.

- For the employee in 2001, they send some email which talk about bankruptcy, correction, and SEC investigation.

- For the CEO it seems in his email we only can find the word bankruptcy. Maybe is because it try to avoid it.

- For the manager in 2001 we find email where we can find the different key words except about the mark-to-market process.

- The trader and Director in 2001 are the only group status to talk about mark-to-market in their email. Maybe is because they are the more involved in it.

- For the trader, in 2000 we find email which talk about SEC investigation as well as loosing money. Maybe it is because they are worry to be spotlight on the fiscal manipulation they did for the Enron company. 

- The vice president talk about bankruptcy, correction and seems to express fear on their email exchange in 2001. They also exchange more about the SEC investigation in 2001 than in 2000.

- The President send email about bankruptcy, correction, express fear, and SEC investigation in 2001 especially. They also exchange on those topics in 2002.

- It is the Managing director and the in house lawyer are the group who exchange the most about mark-to-market even it is still a low number of email, proportionally speaking, they are them which talk the most about it. We can thing they are the group the more imply in this process.

- the in house lawyer exchange especially around the SEC investigation and the bankruptcy maybe because they are invest in this for a legal point of view and/or to discuss with the federal agency for manage the investigation. 

- we can see that in 2001 we see the different status sent few email about with subject talk about the enron event but this topic is the less express compared to the other. We more see it in the email content.

We do the same for the email received:
```{r}
status_email_subject <- df_message_status %>% distinct(date, recipient, status_recipient, subject, reference) %>%
  mutate(#count the number of email which contain at least one word in the list of each topic
    topic_meeting = if_else(str_detect(subject, topic_meeting), 1, 0),
    topic_business_process = if_else(str_detect(subject, topic_business_process), 1, 0),
    topic_core_business = if_else(str_detect(subject, topic_core_business), 1, 0),
...
for(i in seq(status_list)){
  ...
  p <- status_email_subject %>% filter(status_recipient == status) %>%
       ...
       facet_grid(~year)+
         labs(fill = "Email content key words",
           title = paste("Email received about Enron's event and function by", status),
           y = "Email count per category research")+
         scale_fill_manual(values = topic_colors,
                    labels = topic_label)+
        theme(legend.text.position = "bottom",
...
- most of the time the email received are about meeting, except the manager and the president who received more email about the business process than other topics.

- some email are about the enron event in 2001 but that is less than the other topics.

- often it is in the email content we find the key word which are related to the enron event than in the email subject.

- less email have in topis core business compared to the business process especially for the head of the company and the trader suggested they are involved in the fraud. 


```{r}
#global environment cleaning
rm(grid_plot, i, j, month_color, month_label, n, no_legend, p, p3, p4, plot_list, plot_on_the_page, plot_per_section, plots_with_legend, status, status_list,
...
First we look at the number of email send about those subjects
```{r}
...
#pivot the table
person_of_interest_send <-person_of_interest_send %>%
  pivot_longer(
...
```{r fig.width=13}
enron_worker <- c("Jeff Dasovich","Jeffrey Skilling", "Timothy Belden","Lea Fastow","Andrew Fastow",
...
#generating individual plot for each status
for(i in seq(enron_worker)){
  #assign the status to the variable
  worker <- enron_worker[i]
...
From the list find in wikipedia we have find email send by Jeff Dasovich,Jeffrey Skilling, Timothy Belden, Lea Fastow, Andrew Fastow, Vincent Kaminski, Jordan Mintz, Richard Causey, and Greg Whalley which are related to the topics and/or contain the key word research. The count is done for each person for a year.
We can see that:

- most of them have send more email about meeting than other topics except Jordan Mintz and Greg Whalley who send more email about business process than meeting in 2001 compared to the other. We can think that, those person are more concern by the fiscal fradu than the main business of the company.

- Only Jeff Dasovich has sent email about the core business of Enron in 2000 and 2001, Greg Whalley had sent few email about this topic in 2000. This suggest that, the person at the head of the company and/or involved in the fraud don't talk about the main business of Enron.

- Vincent Kaminski and Timothy Belden in 2001 send email about the SEC investigation. We don't see other person of that list sending this type of email except Richard Causey who sent at least one in 2001. Maybe they are of the person who are directly involved in the investigation by the SEC. That could be link whith thier role inside the company.

- Few person send email about the bankruptcy only Timothy Belden, Greg Whalley and Richard Causey send email which contain this words. This suggest they are person involved in the management of this event. We can see Jeff Dasovich sending few email containing this words, because it is an employee this can be because it is inform and/or need to inform other employee about it. 

- Only Jeff Dasovich, Jeffrey Skilling, and Greg Whalley send email who are clearly about the enron event. Perhaps, this topic is more find in the direct email content than the email subject. This suggest this topics are n't clearly speak in the company and it is hidden in the email. 
...
From the list we had at the beginning we find Jeff Dasovich, Jeffrey Skilling, Timothy Belden, Lea Fastow, Andrew Fastow, Vincent Kaminski, Jordan Mintz, Sherron Watkins, Richard Causey, and Greg Whalley. We can observed that:

- all received a high number of email who have in subject meeting. Only Timothy Belden and Jordan Mintz received more email about the business process than meeting. Maybe this is related to their position as trader and manageing director of tax in the company. 

- Jordan Mintz and Vincent kaminski received an important number of email related to the SEC investigation.

- As for the email send, most of them received more email with subject the business process of the company compared to other topics. This pursue to point the fact this Enron worker at the head of the company are more preoccupation about the business process than the core business of the company. 

- Only Greg Whalley, Richard Causey, Vincent Kaminski, Timothy Belden, and Jeff Dasovich received email about the core business of the company.

- The number of email received about the bankruptcy by those person is few. This suggest the subject isn't really treat by email or in the email content we have in this data set. 

- more email topics received are directly about the enron event than those send. In addition, it is the head of the company like Vincent Kaminski who received the higher number of email with this subject. This suggest those email could come from external sender to manage those event by the head of the company.


# !!!List of thing need to be achieved !!!
Read all a last time to see if it is coherent 
make the shiny app
