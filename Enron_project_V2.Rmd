...
email_subject %>% select(year_month, starts_with("sum_topic_")) %>%
  ...
  scale_color_manual(#to get only the customization for the email categories
    values = topic_colors[8:10],
    labels = topic_label[8:10])
```
...
```{r fig.width=13}
enron_worker <- c("Jeff Dasovich","Jeffrey Skilling", "Andrew Baker","Timothy Belden","Lea Fastow","Andrew Fastow",
...
```{r}
person_of_interest_reciveid %>% filter(email_label_recipient == "Jeffrey Skilling")
```
