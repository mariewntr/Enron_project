...
- in the core email we research word such as:
...
Each word/concept will be research individually in the email content to follow the email exchange whose containt them as well as the Enron's worker status imply in those exchange. 
...
email_subject <- df_message_status %>% distinct(date, sender, status_sender, subject, reference) %>%
  ...
  distinct(year_month, sum_topic_meeting, sum_topic_business_process, sum_topic_core_business, sum_email_mark_to_market,
...
email_subject %>% select(year_month, starts_with("sum_topic_")) %>%
  ...
  scale_color_manual(
    values = c("sum_topic_business_process" = "blue",
    "sum_topic_core_business" = "lightgreen",
...
email_subject %>% select(year_month, starts_with("sum_email_")) %>%
  ...
  scale_x_date(date_labels = "%Y-%m", date_breaks = "3 months")+
  scale_color_manual(
    values = c("sum_email_SEC_investigation" = "darkred",
    ...
    labels = c(
    "sum_email_SEC_investigation" = "SEC Investigation",
    "sum_email_10K_report" = "10-K report",
    "sum_email_bankruptcy" = "Bankruptcy",
    "sum_email_correction" = "Correction",
    "sum_email_mark_to_market" = "mark-to-market process",
    "sum_email_losing_money" = "Losing money",
    "sum_email_fear_feeling" = "Fear feeling"))
```
...
- The most important number of exchange about the SEC investigation are realized between July 20001 and January 2002. In parallel we saw also an important number of exchange about a bankruptcy for the company. 
...
status_email_subject <- df_message_status %>% distinct(date, sender, status_sender, subject, reference) %>%
  ...
  #keep one line per year and month
  distinct(year, status_sender, sum_topic_meeting, sum_topic_business_process, sum_topic_core_business, sum_email_mark_to_market,
    sum_email_10K_report,
    sum_email_losing_money,
    sum_email_SEC_investigation,
    sum_email_fear_feeling,
...
for(i in seq(status_list)){
  ...
  p <- status_email_subject %>% filter(status_sender == status) %>%
       ...
       facet_grid(~year)+
         ...
         scale_fill_manual(values = c("sum_email_SEC_investigation" = "darkred",
                                      "sum_email_10K_report" = "pink",
                                      "sum_email_bankruptcy" = "darkgreen",
                                      "sum_email_correction" = "orange",
                                      "sum_email_mark_to_market" = "lavender",
                                      ...
                                      "sum_topic_core_business" = "lightgreen",
                                      "sum_topic_meeting" = "chocolate"),
                    labels = c("sum_email_SEC_investigation" = "SEC Investigation email",
...
*short description of the different player and their role in the enron fraud*
...
person_of_interest_send <- df_message_status %>% filter(str_detect(sender,"andrew.baker|tim.belden|andrew.fastow|lfastow|vkaminski|jordan.mintz|jeff.skilling|sherron.watkins|richard.causey|greg.whalley")) %>%
  mutate(
  ...
  group_by(date, email_label_sender, subject, reference) %>%
  mutate(#count the number of email which contain at least one word in the list of each topic
    topic_meeting = if_else(str_detect(subject, topic_meeting), 1, 0),
    topic_business_process = if_else(str_detect(subject, topic_business_process), 1, 0),
    topic_core_business = if_else(str_detect(subject, topic_core_business), 1, 0),
  ...
  #keep one line per year and month
  distinct(year, email_label_sender, sum_topic_meeting, sum_topic_business_process, sum_topic_core_business,sum_email_mark_to_market,
    sum_email_10K_report,
    sum_email_losing_money,
    sum_email_SEC_investigation,
    sum_email_fear_feeling,
...
For each Enron's worker know for being involved in the different Enron's events we will look at the number of email
```{r fig.width=13}
enron_worker <- c("Jeffrey Skilling", "Andrew Baker","Timothy Belden","Lea Fastow","Andrew Fastow","Vincent Kaminski","Jordan Mintz",
                  "Sherron Watkins","Richard Causey", "Greg Whalley")
    ...
    ggplot(aes(topic_email,value, fill = topic_email))+
       ...
       facet_grid(~year)+
         labs(fill = "Email content key words and topics",
           title = paste("Email send about Enron's event and function by", worker),
           y = "Email count per category research")+
         scale_fill_manual(values = c("sum_email_SEC_investigation" = "darkred",
                                      "sum_email_10K_report" = "pink",
                                      "sum_email_bankruptcy" = "darkgreen",
...
From the list at the beginning only Kenneth Lay isn't find for sending email about those subject.

Next we look at the number of email received about those topics
```{r}
#extract the worker who are interesting to follow and compute the number of email send by them
person_of_interest_reciveid <- df_message_status %>%
  filter(str_detect(recipient,"andrew.baker|tim.belden|andrew.fastow|lfastow|vkaminski|jordan.mintz|jeff.skilling|sherron.watkins|richard.causey|greg.whalley")) %>%
  mutate(
  #identify the person who sent the email
  email_label_recipient = case_when(
    recipient == "kenneth.lay@enron.com" ~ "Kenneth Lay",
    recipient == "jeff.skilling@enron.com" ~ "Jeffrey Skilling",
    recipient == "andrew.baker@enron.com" ~ "Andrew Baker",
    recipient == "tim.belden@enron.com" ~ "Timothy Belden", 
    recipient %in% c("lfastow@pop.pdq.net", "lfastow@pdq.net") ~ "Lea Fastow",
    recipient == "andrew.fastow@enron.com" ~ "Andrew Fastow",
    recipient %in% c("vkaminski@enron.com", "vkaminski@aol.com", "vkaminski@palm.net") ~ "Vincent Kaminski",
    recipient == "jordan.mintz@enron.com" ~ "Jordan Mintz",
    ...
    .default = recipient)) %>% 
  #to compute the number of email sent in each topics by the person whose are directly involved in the Enron scandal
  group_by(date,email_label_recipient, subject, reference) %>%
  mutate(#count the number of email which contain at least one word in the list of each topic
    topic_meeting = if_else(str_detect(subject, topic_meeting), 1, 0),
    topic_business_process = if_else(str_detect(subject, topic_business_process), 1, 0),
    topic_core_business = if_else(str_detect(subject, topic_core_business), 1, 0),
  ...
  #keep one line per year and month
  distinct(year, email_label_recipient, sum_topic_meeting, sum_topic_business_process, sum_topic_core_business,sum_email_mark_to_market,
    sum_email_10K_report,
    sum_email_losing_money,
    sum_email_SEC_investigation,
    sum_email_fear_feeling,
...
From the list at the beginning only Kenneth Lay isn't find for reciving email about those subject.

*make the analyse of the 2 above plot about the email send/received by the enron employee directly involved*
